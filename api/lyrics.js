 ...
 *
 * Multi-provider lyrics fallback chain:
 *   1. lrclib.net  → real synced LRC timestamps (best, free, no key)
 *   2. lyrics.ovh  → plain text, spread evenly across duration
 *
 * This mirrors server/routes/lyrics.js but as a standalone Vercel handler.
 * It is required because the Express server (server/index.js) is NOT deployed
 * to Vercel — only the files in api/ are serverless functions.
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (result) {
      console.log(`[/api/lyrics] lyrics.ovh ✓ "${title}" – ${artist}`);
      res.setHeader('Cache-Control', 'public, max-age=86400');
      return res.json(result);
    }
  ...
  return res.status(404).json({ error: 'Lyrics not found', title, artist, tried: errors });
}
